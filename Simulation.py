...
for t in range (2,nt-1):
    if T_ext[t] < 3:
        ...
        B = np.array([T_ext[t]/Rcond, -q12 - cp*(gap12 + gap23/2 + gap1)*T_ext[t], C*T3[t-1,0]/dt, T_ext[t]/Rcond, -q34 -(gap23/2 + gap34 + gap45/2)*cp*T_ext[t] , C*T3[t-1,1]/dt, T_ext[t]/Rcond, -q56 -(gap45/2 + gap56 + gap6)*cp*T_ext[t], C*T3[t-1,2]/dt]).reshape(-1, 1)
