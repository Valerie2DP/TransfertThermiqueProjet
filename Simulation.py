...
for t in range (2,nt-1):
    ...
    if T_ext[t] < 3:
        ...
        A = np.array([
            ...
            [1/Rconv, -((1/Req1)+(1/Rconv)) - cp*(a23+a32+(gap1 + gap12 + gap23/2)), 1/Req1, cp*(a23 + a32), 0, 0, 0, 0, 0], #Noeud 12
            [0, -1/Req1, ((C/dt)+(1/Req1)), 0, 0, 0, 0, 0, 0], #Noeud 13
            [0, 0, 0, ((1/Rconv)+(1/Rcond)), -1/Rconv, 0, 0, 0, 0], #Noeud 21
            [0, cp*(a32 + a23), 0, 1/Rconv,  -((1/Req2)+(1/Rconv)) + -cp*(a32 + a23 + a45 + a54 + (gap23/2 + gap34 + gap45/2)), 1/Req2, 0, cp*(a45 + a54), 0], #Noeud 22
            [0, 0, 0, 0, -1/Req2, ((C/dt)+(1/Req2)), 0, 0, 0], #Noeud 23
            [0, 0, 0, 0, 0, 0, ((1/Rconv)+(1/Rcond)), -1/Rconv, 0], #Noeud 31
            [0, 0, 0, 0, cp*(a45 + a54), 0, 1/Rconv,  -((1/Req3)+(1/Rconv)) - cp*(a45 + a54 + (gap45/2 + gap56 + gap6)), 1/Req3], #Noeud 32
            [0, 0, 0, 0, 0, 0, 0, -1/Req3, ((C/dt)+(1/Req3))] #Noeud 33
        ], dtype=float)
        B = np.array([T_ext[t]/Rcond, -q12 - cp*(gap12 + gap23/2 + gap1)*T_ext[t], C*T[t-1,2]/dt, T_ext[t]/Rcond, -q34 -(gap23/2 + gap34 + gap45/2)*cp*T_ext[t] , C*T[t-1,5]/dt, T_ext[t]/Rcond, -q56 -(gap45/2 + gap56 + gap6)*cp*T_ext[t], C*T[t-1,8]/dt]).reshape(-1, 1)
